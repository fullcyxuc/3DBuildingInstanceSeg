...
class Dataset:
	...
	def getCroppedInstLabel(self, instance_label, valid_idxs):  # adjust the instance label to make it sort in order
	...
	def trainMerge(self, id):  # id is the index of data with the size of batch
		# locs = []
		locs_float = []
		...
		for i, idx in enumerate(id):
			...
			instance_num.append(inst_num)

			instance_label[np.where(instance_label != -100)] += total_inst_num
			total_inst_num += inst_num

			# locs.append(torch.cat([torch.LongTensor(xyz.shape[0], 1).fill_(i), torch.from_numpy(xyz).long()], 1).unsqueeze(0))
			locs_float.append(torch.from_numpy(xyz_middle).unsqueeze(0))
			feats.append(torch.from_numpy(rgb).unsqueeze(0) + torch.randn(3) * 0.1)
			labels.append(torch.from_numpy(label).unsqueeze(0))
			instance_labels.append(torch.from_numpy(instance_label).unsqueeze(0))

		### merge all the scenes in the batchd
		# locs = torch.cat(locs, 0)  # long (B, N, 1 + 3), the batch item idx is put in locs[:, 0]
		locs_float = torch.cat(locs_float, 0).to(torch.float32)  # float (N, 3)
		feats = torch.cat(feats, 0)  # float (B, N, C)
		labels = torch.cat(labels, 0).long()  # long (B, N)
		instance_labels = torch.cat(instance_labels, 0).long()  # long (B, N)
		instance_num = torch.tensor(instance_num, dtype=torch.int)  # int (B, ) the number of instance for each batch
		# print(id, instance_num, locs_float.size())
		return {'locs_float': locs_float, 'feats': feats, 'labels': labels, 'instance_labels': instance_labels,
	...
	def valMerge(self, id):
		...
		for i, idx in enumerate(id):
			...
			# locs.append(torch.cat([torch.LongTensor(xyz.shape[0], 1).fill_(i), torch.from_numpy(xyz).long()], 1).unsqueeze(0))
			locs_float.append(torch.from_numpy(xyz_middle).unsqueeze(0))
			feats.append(torch.from_numpy(rgb).unsqueeze(0))
			labels.append(torch.from_numpy(label).unsqueeze(0))
			instance_labels.append(torch.from_numpy(instance_label).unsqueeze(0))
		...
		instance_num = torch.tensor(instance_num, dtype=torch.int)
	...
	def testMerge(self, id):
		...
		### merge all the scenes in the batch
		# locs = torch.cat(locs, 0).unsqueeze(0)  # long (N, 1 + 3), the batch item idx is put in locs[:, 0]
		locs_float = torch.cat(locs_float, 0).to(torch.float32).unsqueeze(0)  # float (N, 3)
		feats = torch.cat(feats, 0).unsqueeze(0)  # float (N, C)

		return {'locs_float': locs_float, 'feats': feats}
