...
if opt.data_split != '/media/xue/DATA/xue/scannetv2/test':
...
def f(fn):
    fn2 = fn[:-3] + 'labels.ply'
    fn3 = fn[:-15] + '_vh_clean_2.0.010000.segs.json'  # store the over segmentation (that means may an object can be segmented into many parts) id for each points i.e. seg label
    fn4 = fn[:-15] + '.aggregation.json'  # store info. for each object
    print(fn)
    # if "scene0217_00" in fn:
    #     return
